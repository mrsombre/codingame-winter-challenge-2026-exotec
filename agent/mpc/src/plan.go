...
type BotRoute struct {
	...
	SimBody    []int       // projected body after last planned apple (copied)
	SimDir     int         // projected facing direction
	SimTurnNum int         // projected absolute turn number
}

// EnemyBelief tracks predicted intent for one enemy snake.
type EnemyBelief struct {
	SnIdx      int // index into g.Sn[]
	Target     int // predicted target apple cell (-1 = unknown)
	ETA        int // predicted turns to reach target
	Confidence int // 0-10, increases when prediction matches observed move
	LastDir    int // last observed heading direction (-1 = unknown)
	PrevHead   int // head cell last turn (-1 = first turn)
}
...
type Plan struct {
	...
	EnemyClaimed  []bool // [NCells] apples won by enemy in Voronoi
	Beliefs       [MaxASn]EnemyBelief // persistent enemy beliefs
...
func (p *Plan) Init() {
	...
	p.EnemyClaimed = make([]bool, g.NCells)
	for i := range p.Beliefs {
		p.Beliefs[i] = EnemyBelief{
			SnIdx: i, Target: -1, ETA: -1, Confidence: 0,
			LastDir: -1, PrevHead: -1,
		}
	}
	p.TurnCount = 0
	p.Initialized = false
...
func (p *Plan) Turn() {
	...
	// Prune enemy-claimed apples that have been eaten.
	if p.EnemyClaimed != nil {
		for i := range p.EnemyClaimed {
			if p.EnemyClaimed[i] && !appleAt[i] {
