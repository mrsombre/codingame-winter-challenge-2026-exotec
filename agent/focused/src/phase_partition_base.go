...
const (
	baseRouteMaxDepth    = 8  // max apples per bot in initial sim plan
	baseRouteMaxSteps    = 40 // max steps per bot route (safety cap)
	baseRouteReplayDepth = 15 // max BFS depth in replayPath
)

// phasePartition is the main dispatcher.
func (d *Decision) phasePartitionBase() {
	p := d.P
	...
	if !p.Initialized {
		...
		d.validateRoutesBase()
	}

	d.rebuildPlannedApplesBase()
	d.executeRoutesBase()
}
...
func (d *Decision) greedyFallbackBase(si, snIdx int) {
	g := d.G
	p := d.P
	sn := &g.Sn[snIdx]
...
func (d *Decision) executeRoutesBase() {
	g := d.G
	p := d.P
	...
	for si, snIdx := range d.MySnakes {
		...
		if route.Valid && route.StepCursor < len(route.Steps) {
			...
			d.Assigned[si] = nextRouteAppleBase(route)
			route.StepCursor++
		} else {
			d.greedyFallbackBase(si, snIdx)
		}
	}
}
...
func (d *Decision) validateRoutesBase() {
	p := d.P
	g := d.G
	...
	for si, snIdx := range d.MySnakes {
		...
		if route.StepCursor < len(route.Steps) {
			...
			if sn.Body[0] != step.ExpHead {
				...
				d.replanRouteBase(si, snIdx, route, appleAlive)
			}
		} else {
...
func (d *Decision) replanRouteBase(si, snIdx int, route *BotRoute, appleAlive []bool) {
	g := d.G
	sn := &g.Sn[snIdx]
	...
	for _, ap := range route.AppleSeq {
		if len(route.Steps) >= baseRouteMaxSteps {
			break
		}
		if !appleAlive[ap] {
			continue
		...
		steps, finalBody := d.replayPathBase(sim, curBody, curDir, ap, turnNum, avail)
		if steps == nil {
...
func (d *Decision) rebuildPlannedApplesBase() {
	p := d.P
...
func (d *Decision) planAllRoutesBase() {
	g := d.G
	myN := len(d.MySnakes)
	if myN == 0 {
		return
	...
	for round := 0; round < baseRouteMaxDepth; round++ {
		if len(avail) == 0 {
			break
		}
		...
		for si, snIdx := range d.MySnakes {
			...
			// Use lower threshold than baseRouteMaxDepth so greedy bots don't hoard.
		...
		for _, winner := range assigned {
			...
			steps, finalBody := d.replayPathBase(sim, states[si].body, states[si].dir,
	...
	// Sequential fill: bots with < baseRouteMaxDepth apples get more from remaining pool.
	// Process bots by fewest apples first (most hungry first).
	for fill := 0; fill < baseRouteMaxDepth; fill++ {
		if len(avail) == 0 {
			break
		}
		progress := false
		for si, snIdx := range d.MySnakes {
			if len(d.P.Routes[si].AppleSeq) >= baseRouteMaxDepth {
				continue
			}
			if !states[si].alive {
				continue
...
func (d *Decision) pickBestTargetBase(targets []SimTarget) SimTarget {
	best := 0
	for i, t := range targets {
...
// replayPathBase keeps the original deeper replay horizon used by the
// long-lived base planner. The focused tactical search intentionally uses
// a shorter horizon in replayPath.
func (d *Decision) replayPathBase(
	sim *Sim,
	startBody []int,
	startDir int,
	targetApple int,
	...
	for qi := 0; qi < len(queue) && qi < replayMaxNodes; qi++ {
		cur := queue[qi]
		if cur.dist >= baseRouteReplayDepth {
			continue
		}
		...
		for dir := 0; dir < 4; dir++ {
			...
			if eating {
				return d.buildRouteStepsBase(sim, startBody, newDirs, targetApple, startTurn, avail), bodycp
			}
...
func (d *Decision) buildRouteStepsBase(
	sim *Sim,
	startBody []int,
	dirs []int,
	targetApple int,
...
// nextRouteAppleBase returns the first apple in the route's remaining sequence.
func nextRouteAppleBase(route *BotRoute) int {
	if len(route.AppleSeq) > 0 {
